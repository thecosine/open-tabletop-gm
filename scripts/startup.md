...
Only the trusted GM may translate settled persistent ownership, location, or quantity intent into a strict JSON call to `scripts/inventory_action.py`. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, whole-record `transfer_item`, `consume_item`, and `split_stack`; `combine_stacks` is not yet supported. Persistent quantity intent includes consume one, expend one, use up one, use one and remove it from inventory, split the stack, and separate N from the stack. Explicit transfer intent includes give permanent ownership of the iron key to Sassafras, transfer the rope from Mythlon's inventory to Sassafras, move ownership of this item to Sassafras, add this item to Sassafras's inventory and remove it from Mythlon's, and Sassafras takes permanent ownership of the item. Resolve ownership, exact source, destination, item, quantity, destination group and requested container, permanence, and persistence intent first; ask for clarification rather than guessing.

Do not call the inventory command for pick up, hold, hand temporarily, inspect, examine, count, search, drink, eat, use, apply, fire, shoot, throw, spend, or hand over. Firing an arrow does not automatically decrement ammunition. Ambiguous phrases including take it, give her the item, put it away, use a potion, get rid of it, move the arrows, I drink a potion, Spend one, Hand over an arrow, Split some arrows, hand Sassafras the key, let Sassafras hold this, pass her an arrow, show her the item, lend her the bow, she carries it for now, give her some arrows, and split the arrows between them require clarification. Temporary handoff is narration, lending is not ownership transfer, and partial quantities require `split_stack` first. Transfer does not equip, unequip, attune, or unattune; currency-group transfer is not yet supported. Splitting the 12 matched arrows that exist only in Mythlon's War Arrows notes requires explicit GM confirmation and must not rewrite names or notes. Inventory actions do not equip or change attunement; equipment and attunement actions do not add or remove ownership.
