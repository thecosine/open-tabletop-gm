...
Only the trusted GM may translate settled persistent ownership, location, or quantity intent into a strict JSON call to `scripts/inventory_action.py`. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, `consume_item`, and `split_stack`; `combine_stacks` is not yet supported. Persistent quantity intent includes consume one, expend one, use up one, use one and remove it from inventory, split the stack, and separate N from the stack. Resolve ownership, exact item, quantity, destination, and persistence intent first; ask for clarification rather than guessing.

Do not call the inventory command for pick up, hold, hand temporarily, inspect, examine, count, search, drink, eat, use, apply, fire, shoot, throw, spend, or hand over. Firing an arrow does not automatically decrement ammunition. Ambiguous phrases including take it, give her the item, put it away, use a potion, get rid of it, move the arrows, I drink a potion, Spend one, Hand over an arrow, and Split some arrows require clarification. Splitting the 12 matched arrows that exist only in Mythlon's War Arrows notes requires explicit GM confirmation and must not rewrite names or notes. Inventory actions do not equip or change attunement; equipment and attunement actions do not add or remove ownership.
