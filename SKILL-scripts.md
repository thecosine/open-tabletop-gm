...
Trusted GM-only command for explicit persistent ownership, item-location, and quantity changes. It accepts one strict JSON object on stdin. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, `consume_item`, and `split_stack`; `combine_stacks` is not yet supported. It shares `inventory-state.json`, the campaign revision, lock, request-ID namespace, audit history, atomic persistence, and projection-only display refresh with equipment and attunement actions. Inventory request IDs begin with `inventory-`.
...
Invoke quantity actions only for settled persistent intent such as `consume one`, `expend one`, `use up one`, `use one and remove it from inventory`, `split the stack`, or `separate N from the stack`. Ordinary `pick up`, `drink`, `eat`, `use`, `apply`, `fire`, `shoot`, `throw`, `spend`, or `hand over` narration does not invoke them; firing an arrow does not automatically decrement ammunition. Ask for clarification for `I drink a potion`, `Use a potion`, `Spend one`, `Hand over an arrow`, `Split some arrows`, and `Split off the 12 matched arrows`. The matched arrows remain a notes-described subset of Mythlon's 1,106 War Arrows and require explicit GM confirmation before a split; the action must not rewrite names or notes. Equipment, attunement, and inventory persistence remain separate, and ambiguous intent requires clarification.
