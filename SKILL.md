...
Invoke `scripts/inventory_action.py` only after the GM has adjudicated explicit persistent ownership, location, or quantity intent. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, whole-record `transfer_item`, `consume_item`, and `split_stack`; `combine_stacks` is not yet supported. Valid intent includes `add to inventory`, `acquire into inventory`, `take into inventory`, `discard`, `destroy`, `remove from inventory`, `move into container`, `take out of container`, `consume one`, `expend one`, `use up one`, `use one and remove it from inventory`, `split the stack`, `separate N from the stack`, `give permanent ownership of the iron key to Sassafras`, `transfer the rope from Mythlon's inventory to Sassafras`, `move ownership of this item to Sassafras`, `add this item to Sassafras's inventory and remove it from Mythlon's`, and `Sassafras takes permanent ownership of the item`. Require the exact source, destination, item, complete current record, ownership, permanence, exact quantity when applicable, and explicit destination group and requested container before translating the intent into strict JSON.

Ordinary narration does **not** mutate inventory. Do not invoke the command for `pick up`, `hold`, temporary `hand`, `inspect`, `examine`, `count`, `search`, `drink`, `eat`, `use`, `apply`, `fire`, `shoot`, `throw`, `spend`, or `hand over`. Firing an arrow does not automatically decrement ammunition. Ask for clarification for phrases such as `take it`, `give her the item`, `put it away`, `use a potion`, `get rid of it`, `move the arrows`, `I drink a potion`, `Spend one`, `Hand over an arrow`, `Split some arrows`, `hand Sassafras the key`, `let Sassafras hold this`, `pass her an arrow`, `show her the item`, `lend her the bow`, `she carries it for now`, `give her some arrows`, or `split the arrows between them`. Temporary handoff is narration and lending is not an ownership transfer. Partial quantities require `split_stack` first. Whole-record transfer does not equip, unequip, attune, or unattune; currency-group transfer is not yet supported. Splitting the 12 matched arrows described only in Mythlon's War Arrows notes requires explicit GM confirmation before invoking the action; splitting never rewrites names or notes. Inventory, equipment, and attunement actions remain separate; none implies either of the others.
