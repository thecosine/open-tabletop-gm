...
Invoke `scripts/inventory_action.py` only after the GM has adjudicated explicit persistent ownership, location, quantity, final item-identification, or identified-pouch currency-conversion intent. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, whole-record `transfer_item`, `consume_item`, `split_stack`, `identify_item`, and `convert_item_to_currency`; `combine_stacks` is not yet supported. Valid intent includes `add to inventory`, `acquire into inventory`, `take into inventory`, `discard`, `destroy`, `remove from inventory`, `move into container`, `take out of container`, `consume one`, `expend one`, `use up one`, `use one and remove it from inventory`, `split the stack`, `separate N from the stack`, `give permanent ownership of the iron key to Sassafras`, `transfer the rope from Mythlon's inventory to Sassafras`, `move ownership of this item to Sassafras`, `add this item to Sassafras's inventory and remove it from Mythlon's`, `Sassafras takes permanent ownership of the item`, `identify this pouch as ...`, `record this item as ...`, `reveal this item to be ...`, `update the unidentified item to ...`, and `the GM confirms this item is ...`. Require the exact source, item, complete current record, ownership, and settled persistence intent; also require destination and quantity details for operations that use them, and an exact stable item plus complete final identified record for identification.

Settled conversion intent includes `convert this identified pouch into 12 sp and 7 cp`, `add these confirmed pouch contents to currency and remove the pouch`, `bank these coins as 12 sp and 7 cp`, `exchange this pouch into standard currency at denomination value`, and `the merchant accepts these coins at face value; remove the pouch and add the exact denominations`. Require one exact stable source item ID, exact amounts for all five canonical denominations (`cp`, `sp`, `ep`, `gp`, and `pp`, including zeros), and explicit `remove_entire_record` source disposition. All mints are equal by denomination; there are no exchange rates, fees, discounts, sale mechanics, assay mechanics, or mint tracking. The source pouch is removed entirely. Empty-pouch preservation, partial conversion, bulk conversion, ownership transfer within conversion, and amounts inferred from notes are unsupported.

Ordinary narration does **not** mutate inventory. Do not invoke the command for `pick up`, `hold`, temporary `hand`, `inspect the pouch`, `count the coins`, `appraise the coins`, `cast identify`, `study the item`, `what is this?`, `detect magic`, `these coins are worth ...`, `this pouch contains ...`, `sell the pouch`, `trade the pouch`, `take these to a bank`, `a merchant might accept these`, `gold is gold`, `drink`, `eat`, `use`, `apply`, `fire`, `shoot`, `throw`, `spend`, or `hand over`. Inspection does not convert currency, `identify_item` does not convert currency, and recording pouch contents does not convert currency; only settled explicit conversion mutates currency. Ask for clarification when the exact stable source item, exact denomination amounts, or source disposition is missing; conversion versus sale is unclear; the request is speculative; or multiple pouches are requested. Temporary handoff is narration and lending is not an ownership transfer. Partial quantities require `split_stack` first. Whole-record transfer does not equip, unequip, attune, or unattune; currency-group transfer is not yet supported. Splitting the 12 matched arrows described only in Mythlon's War Arrows notes requires explicit GM confirmation before invoking the action; splitting never rewrites names or notes. Inventory, equipment, and attunement actions remain separate; none implies either of the others.
