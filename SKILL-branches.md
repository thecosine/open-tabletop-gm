...
2. If and only if the player clearly expresses explicit persistent inventory ownership, location, or quantity intent, adjudicate ownership, exact item, quantity, destination, and persistence first; then translate it into strict JSON and run `scripts/inventory_action.py`. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, `consume_item`, and `split_stack`; `combine_stacks` is not yet supported. Quantity intent includes `consume one`, `expend one`, `use up one`, `use one and remove it from inventory`, `split the stack`, and `separate N from the stack`. Do not run it for ordinary narration such as `pick up`, `hold`, temporary `hand`, `inspect`, `examine`, `count`, `search`, `drink`, `eat`, `use`, `apply`, `fire`, `shoot`, `throw`, `spend`, or `hand over`; firing an arrow never automatically decrements ammunition. Ask for clarification for `take it`, `give her the item`, `put it away`, `use a potion`, `get rid of it`, `move the arrows`, `I drink a potion`, `Spend one`, `Hand over an arrow`, or `Split some arrows`. Require explicit GM confirmation before splitting the 12 matched arrows that exist only as a notes-described subset of Mythlon's War Arrows; do not rewrite names or notes. Inventory, equipment, and attunement actions remain separate.
