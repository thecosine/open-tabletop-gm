...
2. If and only if the player clearly expresses explicit persistent inventory ownership, location, quantity, or settled final identification intent, adjudicate ownership, exact source, destination, item, quantity, destination group and requested container, permanence, and persistence first; then translate it into strict JSON and run `scripts/inventory_action.py`. Supported operations are `add_item`, whole-record `remove_item`, whole-record `move_item`, whole-record `transfer_item`, `consume_item`, `split_stack`, and `identify_item`; `combine_stacks` is not yet supported. Explicit transfer intent includes `give permanent ownership of the iron key to Sassafras`, `transfer the rope from Mythlon's inventory to Sassafras`, `move ownership of this item to Sassafras`, `add this item to Sassafras's inventory and remove it from Mythlon's`, and `Sassafras takes permanent ownership of the item`. Quantity intent includes `consume one`, `expend one`, `use up one`, `use one and remove it from inventory`, `split the stack`, and `separate N from the stack`. Settled identification intent includes `identify this pouch as ...`, `record this item as ...`, `reveal this item to be ...`, `update the unidentified item to ...`, and `the GM confirms this item is ...`; require an exact stable item and complete final identified record. Do not run it for ordinary narration such as `pick up`, `hold`, temporary `hand`, `inspect`, `examine`, `appraise`, `cast identify`, `study the item`, `what is this?`, `detect magic`, `check the coins`, `count`, `search`, `drink`, `eat`, `use`, `apply`, `fire`, `shoot`, `throw`, `spend`, or `hand over`; spell use and skill checks never automatically mutate inventory, and firing an arrow never automatically decrements ammunition. Ask for clarification for `take it`, `give her the item`, `put it away`, `use a potion`, `get rid of it`, `move the arrows`, `I drink a potion`, `Spend one`, `Hand over an arrow`, `Split some arrows`, `hand Sassafras the key`, `let Sassafras hold this`, `pass her an arrow`, `show her the item`, `lend her the bow`, `she carries it for now`, `give her some arrows`, `split the arrows between them`, `identify the pouch`, `identify all the pouches`, `I think it might be ...`, or `maybe it is ...`. Only an adjudicated final identity mutates. Bulk identification, correction of already-identified records, partial-stack identification, equipment-property revelation, attunement-property revelation, and currency conversion are unsupported. Temporary handoff is narration, lending is not ownership transfer, and partial quantities require `split_stack` first. Transfer does not equip, unequip, attune, or unattune; currency-group transfer is not yet supported. Require explicit GM confirmation before splitting the 12 matched arrows that exist only as a notes-described subset of Mythlon's War Arrows; do not rewrite names or notes. Inventory, equipment, and attunement actions remain separate.
   `convert_item_to_currency` is also supported for settled explicit conversion of one identified carried singleton pouch. Valid intent includes `convert this identified pouch into 12 sp and 7 cp`, `add these confirmed pouch contents to currency and remove the pouch`, `bank these coins as 12 sp and 7 cp`, `exchange this pouch into standard currency at denomination value`, and merchant acceptance at face value with explicit removal and exact denominations. Require one exact stable source item ID, exact integer amounts for all five denominations (`cp`, `sp`, `ep`, `gp`, `pp`, including zeros), and exact `source_disposition: remove_entire_record`. Inspection does not convert currency, `identify_item` does not convert currency, and recording pouch contents does not convert currency. Do not convert for `inspect the pouch`, `count the coins`, `appraise the coins`, `identify the pouch`, `these coins are worth ...`, `this pouch contains ...`, `sell the pouch`, `trade the pouch`, `take these to a bank`, `a merchant might accept these`, or `gold is gold`. Clarify a missing exact source, denomination amount, or source disposition; unclear conversion versus sale; speculative requests; and multiple pouches. All mints are equal by denomination. There are no exchange rates, fees, discounts, sale mechanics, assay mechanics, or mint tracking. Remove the source pouch entirely; empty-pouch preservation, bulk conversion, partial conversion, and ownership transfer within conversion are unsupported. Never infer amounts from notes.
