...
2. If and only if the player clearly expresses explicit persistent inventory ownership or location intent, adjudicate ownership, exact item, quantity, destination, and persistence first; then translate it into strict JSON and run `scripts/inventory_action.py`. Initial operations are `add_item`, whole-record `remove_item`, and whole-record `move_item`. Valid phrases include `add to inventory`, `acquire into inventory`, `take into inventory`, `discard`, `destroy`, `remove from inventory`, `move into container`, and `take out of container`. Do not run it for ordinary narration such as `pick up`, `hold`, temporary `hand`, `inspect`, `examine`, `count`, `search`, `use`, `drink`, `fire`, or `throw`. Ask for clarification for `take it`, `give her the item`, `put it away`, `use a potion`, `get rid of it`, or `move the arrows`.
